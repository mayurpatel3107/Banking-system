...
<body style="background: linear-gradient(#f8f9fa,#084298);">
    ...
    <nav class="navbar navbar-expand-lg navbar-white bg-white fixed fixed-top"
        style="font-family:candara; font-size:18px; box-shadow: 0px 0px 10px 0px black;">
        <div class="container-fluid">
            <a href="index.php"><img src="bitcoin.png" alt=""
                    style="height: 50px; margin-left:25px; margin-top:10px;"></a>
    ...
    <center style="margin:120px 0px 20px 0px;">
        <div class="container" style="margin-top:2%;">
            ...
            <div class="" style=" backdrop-filter: blur(5px);  border-radius:5px;  ">
                <form action="send_money.php" method="post">
                    <table>
                        <tr>
                            <td><input type="text" class="formin form-control" name="account1" id=""
                        ...
                        <tr>
                            <td><input type="text" class="formin form-control" name="account2" id=""
                                    placeholder="Reciever's Account Number"></td>
                        </tr>
                    </table>
                    <br><input class="btn mybtn btn-outline-light" type="submit" value="Transfer"><br><br>
                    <p style="color:white;">Want to check your account balance? check <a href="check_blc.php">here</a></p>
                </form>
            </div>
        </div>
...
if($_SERVER['REQUEST_METHOD']== 'POST'){
    ...
    $sender = $_POST['account1'];
    $amount = $_POST['amount'];
    $reciever = $_POST['account2'];
   
    
    $checkbalancequery = "SELECT balance FROM users where account='$sender'";
    $checkbalance = mysqli_query($conn, $checkbalancequery);
    $ava_balance = mysqli_fetch_assoc($checkbalance)['balance'];
    if($ava_balance >= $amount){
    $sql1 = "UPDATE users SET balance= balance-$amount WHERE account='$sender'";
    $sql2 = "UPDATE users SET balance= balance+$amount WHERE account='$reciever'";
    $result1 = mysqli_query($conn, $sql1);
    $result2 = mysqli_query($conn, $sql2);
    if($result1 && $result2){
        echo '<div class="alert alert-success align-items-center numner-center" style="width:50%;" role="alert">
        <div>   
    ...
    echo '<div class="alert alert-danger align-items-center number  -center" style="width:50%;" role="alert">
        <div>   
